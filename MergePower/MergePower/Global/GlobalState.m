...
@property (nonatomic, readwrite) MyGameType gameType;
...
- (BOOL)isLevel:(NSInteger)level1 mergeableWithLevel:(NSInteger)level2 {
    if (self.gameType == MyGameTypeFibonacci) return abs((int)level1 - (int)level2) == 1;
...
- (NSInteger)mergeLevel:(NSInteger)level1 withLevel:(NSInteger)level2 {
    ...
    if (self.gameType == MyGameTypeFibonacci) {
...
- (NSInteger)valueForLevel:(NSInteger)level {
    if (self.gameType == MyGameTypeFibonacci) {
        ...
        NSInteger value = 1;
        NSInteger base = self.gameType == MyGameTypePowerOf2 ? 2 : 3;
        for (NSInteger i = 0; i < level; i++) {
            value *= base;
        }
        return value;
...
- (CGPoint)locationOfPosition:(MyPosition)position {
...
- (CGFloat)xLocationOfPosition:(MyPosition)position {
    return position.y * (GSTATE.tileSize + GSTATE.borderWidth) + GSTATE.borderWidth;
}


- (CGFloat)yLocationOfPosition:(MyPosition)position {
